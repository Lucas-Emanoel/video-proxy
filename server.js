...
app.get(\'/api/proxy\', async (req, res) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Erro do servidor de destino: Status ${response.status} ${response.statusText} para a URL ${url}. Corpo da resposta: ${errorBody}`);
    ...
    console.error(\'Erro no proxy (detalhes):\', error.message);
    res.status(500).json({
